$(function() {
	...
	$("#searchAddress").click(function(event) {
		if ($("#gunDDiv").css("display") == 'block') {
			...
			for (var i = 0; i < provinceList.length; i++) {
				html += "<li class='mouseGesture provinceDiv'><input type='hidden' name='proNo' value =' " + i + "'/>" +
					provinceList[i].name + "</li>";
			}
			$("#wraap1").html(html);
			console.log(provinceList[0].cityList);
		}
	...
	//点击省份 二级联动
	$("#wraap1").on("click", "li", function(event) {
		event.stopPropagation();
		...
		console.log(proText);
		var i = parseInt($(this).find("input").val());
		console.log(i);
		$(".cityDiv").html("");
		...
		if (i == 0 || i == 1 || i == 2) {
			...
			var html = "";
			for(var j = 0;j < provinceList[i].cityList[0].areaList.length;j++){
				html += "<p>"+provinceList[i].cityList[0].areaList[j]+"</p>";
			}
		}else if(i == 3){
			var html = "";
			for(var j = 0;j < provinceList[i].cityList[0].areaList.length;j++){
			...
			for(var j = 0;j < provinceList[i].cityList[1].areaList.length;j++){
				html += "<p>"+provinceList[i].cityList[1].areaList[j]+"</p>";
			}
		}else{
			var html = "";
			for(var j = 0;j < provinceList[i].cityList.length;j++){
				html += "<p>"+provinceList[i].cityList[j].name+"</p>";
			}
		}
		$(".cityDiv").html(html);
		$(".cityDiv p").addClass("mouseGesture");
	});
	$(".cityDiv").on("click","p",function(){
		var city = $(this).text();
		console.log(city);
		$(".proText").text(city);
		$.cookie("city",city);
...
var provinceList = [{
	...
	cityList: [{
		name: '市辖区',
		areaList: ['东城区', '西城区', '朝阳区', '丰台区', '石景山区', '海淀区', '门头沟区', '房山区', '通州区', '顺义区', '昌平区', '大兴区', '怀柔区', '平谷区',
			'密云区', '延庆区'
		]
	}]
}, {
	...
	cityList: [{
		name: '市辖区',
		areaList: ['黄浦区', '徐汇区', '长宁区', '静安区', '普陀区', '闸北区', '虹口区', '杨浦区', '闵行区', '宝山区', '嘉定区', '浦东新区', '金山区', '松江区',
			'青浦区', '奉贤区', '崇明区'
		]
	}]
}, {
	...
	cityList: [{
		name: '市辖区',
		areaList: ['和平区', '河东区', '河西区', '南开区', '河北区', '红桥区', '海新区', '东丽区', '西青区', '津南区', '北辰区', '武清区', '宝坻区', '宁河区',
			'静海区', '蓟州区'
		]
	}]
}, {
	...
	cityList: [{
		name: '市辖区',
		areaList: ['万州区', '涪陵区', '渝中区', '大渡口区', '江北区', '沙坪坝区', '九龙坡区', '南岸区', '北碚区', '渝北区', '巴南区', '黔江区', '长寿区', '綦江区',
			'潼南区', '铜梁区', '大足区', '荣昌区', '璧山区', '梁平区', '江津区', '合川区', '永川区', '南川区', '武隆区', '开州区'
		]
	}, {
		name: '县',
		areaList: ['城口县', '丰都县', '垫江县', '忠县', '云阳县', '奉节县', '巫山县', '巫溪县', '石柱土家族自治县', '秀山土家族苗族自治县', '酉阳土家族苗族自治县',
			'彭水苗族土家族自治县'
		]
	}]
}, {
	...
	cityList: [{
		name: '成都市',
		areaList: ['锦江区', '青羊区', '金牛区', '武侯区', '成华区', '龙泉驿区', '青白江区', '新都区', '郫都区', '双流区', '温江县', '金堂县', '大邑县', '蒲江县',
			'新津县', '都江堰市', '彭州市', '邛崃市', '崇州市'
		]
	}, {
		name: '自贡市',
		...
		name: '甘孜藏族自治州',
		areaList: ['康定市', '泸定县', '丹巴县', '九龙县', '雅江县', '道孚县', '炉霍县', '甘孜县', '新龙县', '德格县', '白玉县', '石渠县', '色达县', '理塘县',
			'巴塘县', '乡城县', '稻城县', '得荣县'
		]
	}, {
		name: '凉山彝族自治州',
	...
	cityList: [{
		...
		name: '遵义市',
		areaList: ['红花岗区', '汇川区', '播州区', '桐梓县', '绥阳县', '正安县', '道真仡佬族苗族自治县', '务川仡佬族苗族自治县', '凤冈县', '湄潭县', '余庆县', '习水县',
			'赤水市', '仁怀市'
		]
	}, {
		name: '安顺市',
	...
	cityList: [{
		name: '昆明市',
		areaList: ['五华区', '盘龙区', '官渡区', '西山区', '东川区', '呈贡区', '晋宁区', '富民县', '宜良县', '石林彝族自治县', '嵩明县', '禄劝彝族苗族自治县',
			'寻甸回族彝族自治县', '安宁市'
		]
	}, {
		name: '曲靖市',
		...
		name: '普洱市',
		areaList: ['思茅区', '宁洱哈尼族彝族自治县', '墨江哈尼族自治县', '景东彝族自治县', '景谷傣族彝族自治县', '镇沅彝族哈尼族拉祜族自治县', '江城哈尼族彝族自治县', '孟连傣族拉祜族佤族自治县',
			'澜沧拉祜族自治县', '西盟佤族自治县'
		]
	}, {
		name: '临沧市',
		...
		name: '红河哈尼族彝族自治州',
		areaList: ['个旧市', '开远市', '蒙自市', '屏边苗族自治县', '建水县', '石屏县', '弥勒市', '泸西县', '元阳县', '红河县', '金平苗族瑶族傣族自治县', '绿春县',
			'河口瑶族自治县'
		]
	}, {
		name: '文山壮族苗族自治州',
	...
	cityList: [{
		...
		name: '日喀则市',
		areaList: ['桑珠孜区', '南木林县', '江孜县', '定日县', '萨迦县', '拉孜县', '昂仁县', '谢通门县', '白朗县', '仁布县', '康马县', '定结县', '仲巴县', '亚东县',
			'吉隆县', '聂拉木县', '萨嘎县', '岗巴县'
		]
	}, {
		name: '那曲地区',
	...
	cityList: [{
		...
		name: '洛阳市',
		areaList: ['老城区', '西工区', '廛河回族区', '涧西区', '吉利区', '洛龙区', '孟津县', '新安县', '栾川县', '嵩县', '汝阳县', '宜阳县', '洛宁县', '伊川县',
			'偃师市'
		]
	}, {
		name: '平顶山市',
	...
	cityList: [{
		...
		name: '怀化市',
		areaList: ['鹤城区', '中方县', '沅陵县', '辰溪县', '溆浦县', '会同县', '麻阳苗族自治县', '新晃侗族自治县', '芷江侗族自治县', '靖州苗族侗族自治县', '通道侗族自治县',
			'洪江市'
		]
	}, {
		name: '娄底市',
	...
	cityList: [{
		...
		name: '桂林市',
		areaList: ['秀峰区', '叠彩区', '象山区', '七星区', '雁山区', '临桂区', '阳朔县', '临桂区', '灵川县', '全州县', '兴安县', '永福县', '灌阳县', '龙胜各族自治县',
			'资源县', '平乐县', '荔蒲县', '恭城瑶族自治县'
		]
	}, {
		name: '梧州市',
	...
	cityList: [{
		name: '石家庄市',
		areaList: ['长安区', '藁城区', '鹿泉区', '栾城区', '桥西区', '新华区', '井陉矿区', '裕华区', '井陉县', '正定县', '行唐县', '灵寿县', '高邑县', '深泽县',
			'赞皇县', '无极县', '平山县', '元氏县', '赵县', '辛集市', '晋州市', '新乐市'
		]
	}, {
		name: '唐山市',
		...
		name: '邯郸市',
		areaList: ['邯山区', '丛台区', '复兴区', '峰峰矿区', '肥乡区', '永年区', '临漳县', '成安县', '大名县', '涉县', '磁县', '邱县', '鸡泽县', '广平县', '馆陶县',
			'魏县', '曲周县', '武安市'
		]
	}, {
		name: '邢台市',
		areaList: ['桥东区', '桥西区', '邢台县', '临城县', '内丘县', '柏乡县', '隆尧县', '任县', '南和县', '宁晋县', '巨鹿县', '新河县', '广宗县', '平乡县', '威县',
			'清河县', '临西县', '南宫市', '沙河市'
		]
	}, {
		name: '保定市',
		areaList: ['竞秀区', '莲池区', '满城区', '清苑区', '涞水县', '阜平县', '徐水区', '定兴县', '唐县', '高阳县', '容城县', '涞源县', '望都县', '安新县', '易县',
			'曲阳县', '蠡县', '顺平县', '博野县', '雄县', '涿州市', '定州市', '安国市', '高碑店市'
		]
	}, {
		name: '张家口市',
		areaList: ['桥东区', '桥西区', '宣化区', '下花园区', '万全区', '崇礼区', '张北县', '康保县', '沽源县', '尚义县', '蔚县', '阳原县', '怀安县', '怀来县',
			'涿鹿县', '赤城县'
		]
	}, {
		name: '承德市',
		...
		name: '沧州市',
		areaList: ['新华区', '运河区', '沧县', '青县', '东光县', '海兴县', '盐山县', '肃宁县', '南皮县', '吴桥县', '献县', '孟村回族自治县', '泊头市', '任丘市',
			'黄骅市', '河间市'
		]
	}, {
		name: '廊坊市',
	...
	cityList: [{
		...
		name: '临汾市',
		areaList: ['尧都区', '曲沃县', '翼城县', '襄汾县', '洪洞县', '古县', '安泽县', '浮山县', '吉县', '乡宁县', '大宁县', '隰县', '永和县', '蒲县', '汾西县',
			'侯马市', '霍州市'
		]
	}, {
		name: '吕梁市',
	...
	cityList: [{
		...
		name: '呼伦贝尔市',
		areaList: ['海拉尔区', '阿荣旗', '莫力达瓦达斡尔族自治旗', '鄂伦春自治旗', '鄂温克族自治旗', '陈巴尔虎旗', '新巴尔虎左旗', '新巴尔虎右旗', '满洲里市', '牙克石市', '扎兰屯市',
			'额尔古纳市', '根河市'
		]
	}, {
		name: '巴彦淖尔市',
	...
	cityList: [{
		...
		name: '赣州市',
		areaList: ['章贡区', '赣县区', '南康区', '信丰县', '大余县', '上犹县', '崇义县', '安远县', '龙南县', '定南县', '全南县', '宁都县', '于都县', '兴国县',
			'会昌县', '寻乌县', '石城县', '瑞金市'
		]
	}, {
		name: '吉安市',
	...
	cityList: [{
		name: '哈尔滨市',
		areaList: ['道里区', '南岗区', '道外区', '香坊区', '平房区', '松北区', '呼兰区', '阿城区', '双城区', '依兰县', '方正县', '宾县', '巴彦县', '木兰县', '通河县',
			'延寿县', '尚志市', '五常市'
		]
	}, {
		name: '齐齐哈尔市',
		areaList: ['龙沙区', '建华区', '铁锋区', '昂昂溪区', '富拉尔基区', '碾子山区', '梅里斯达斡尔族区', '龙江县', '依安县', '泰来县', '甘南县', '富裕县', '克山县',
			'克东县', '拜泉县', '讷河市'
		]
	}, {
		name: '鸡西市',
		...
		name: '伊春市',
		areaList: ['伊春区', '南岔区', '友好区', '西林区', '翠峦区', '新青区', '美溪区', '金山屯区', '五营区', '乌马河区', '汤旺河区', '带岭区', '乌伊岭区', '红星区',
			'上甘岭区', '嘉荫县', '铁力市'
		]
	}, {
		name: '佳木斯市',
	...
	cityList: [{
		...
		name: '省直辖县级行政单位',
		areaList: ['五指山市', '琼海市', '文昌市', '万宁市', '东方市', '定安县', '屯昌县', '澄迈县', '临高县', '白沙黎族自治县', '昌江黎族自治县', '乐东黎族自治县',
	...
	cityList: [{
		name: '台北市',
		areaList: ['中区', '东区', '南区', '西区', '北区', '北屯区', '西屯区', '南屯区', '太平区', '大里区', '雾峰区', '乌日区', '丰原区', '后里区', '潭子区',
			'大雅区', '神冈区', '石冈区', '东势区', '新社区', '和平区', '大肚区', '沙鹿区', '龙井区', '梧栖区', '清水区', '大甲区', '外埔区', '大安区'
		]
	}, {
		name: '新北市',
		areaList: ['板桥区', '汐止区', '新店区', '永和区', '中和区', '土城区', '树林区', '三重区', '新庄区', '芦洲市', '瑞芳区', '三峡区', '莺歌区', '淡水区',
			'万里区', '金山区', '深坑区', '石碇区', '平溪区', '双溪区', '贡寮区', '坪林区', '乌来区', '泰山区', '林口区', '五股区', '八里区', '三芝区', '石门区'
		]
	}, {
		name: '台中市',
		areaList: ['中区', '东区', '南区', '西区', '北区', '北屯区', '西屯区', '南屯区', '太平区', '大里区', '雾峰区', '乌日区', '丰原区', '后里区', '潭子区',
		...
		name: '台南市',
		areaList: ['中西区', '东区', '南区', '北区', '安平区', '安南区', '永康区', '归仁区', '新化区', '左镇区', '玉井区', '楠西区', '南化区', '仁德区', '关庙区',
			'龙崎区', '官田区', '麻豆区', '佳里区', '西港区', '七股区', '将军区', '学甲区', '北门区', '新营区', '后壁区', '白河区', '东山区', '六甲区', '下营区', '柳营区',
			'盐水区', '善化区', '大内区', '山上区', '新市区', '安定区'
		]
	}, {
		name: '高雄市',
		areaList: ['楠梓区', '左营区', '鼓山区', '三民区', '盐埕区', '前金区', '新兴区', '苓雅区', '前镇区', '旗津区', '小港区', '凤山区', '大寮区', '鸟松区',
			'林园区', '仁武区', '大树区', '大社区', '冈山区', '路竹区', '桥头区', '梓官区', '弥陀区', '永安区', '燕巢区', '阿莲区', '茄萣区', '湖内区', '旗山区', '美浓区',
			'内门区', '杉林区', '甲仙区', '六龟区', '茂林区', '桃源区', '那玛夏区'
		]
	}]
}, {
	name: '香港',
